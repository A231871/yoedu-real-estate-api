...
BEGIN
  ...
  INSERT INTO listings (
    ...
    listing_type, title, slug, description, area, bedrooms, bathrooms, floors, status
  ) VALUES
    -- 1. Apartment for rent in Hanoi
    (uuidv7(), owner1, agent1, 2, 'Số 12 Nguyễn Chí Thanh, Ba Đình', '00004',
    ...
    -- 2. House for sale
    (uuidv7(), owner2, NULL, 3, 'Ngõ 45 Tô Ngọc Vân, Tây Hồ', '00103',
  ...
  -- ============================================================
  -- SEED: Listing Prices (latest price for each listing)
  -- ============================================================

  INSERT INTO listing_prices (id, listing_id, amount_vnd)
  SELECT uuidv7(), l.id,
    CASE
      WHEN l.listing_type = 'FOR_RENT' THEN
        CASE
          WHEN l.area < 50 THEN 5000000
          WHEN l.area < 100 THEN 12000000
          ELSE 25000000
        END
      ELSE -- FOR_SALE
        (l.area * 45000000)::numeric
    END
  FROM listings l
  WHERE NOT EXISTS (SELECT 1 FROM listing_prices lp WHERE lp.listing_id = l.id);
